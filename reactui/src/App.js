...
class App extends Component {
  ...
  componentWillMount() {
    ...
    var _addresses = Array.from(client.eth.accounts);
    var _balances = [];
    for (var i = 0; i < _addresses.length; i++) {
      _balances.push(client.eth.getBalance(_addresses[i]));
    }
  ...
  handleClick(event) {
    ...
    for (var i = 0; i < this.state.balances.length; i++) {
      this.state.balances[i] = client.eth.getBalance(this.state.addresses[i]);
    }  
  }
